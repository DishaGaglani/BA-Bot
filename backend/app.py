...
async def mock_predict(request: Request):
    ...
    if streaming:
        def mock_event_generator():
            for line in generate_mock_stream_lines():
                yield f"{line}\n\n"

        return StreamingResponse(mock_event_generator(), media_type="text/event-stream")
    else:
        return {"text": build_mock_response_text(question_prompt)}
    ...
    def event_generator():
        ...
        for attempt in range(2):
            try:
                ...
                print("[PREDICTION FALLBACK] Generating local mock response in-process (no HTTP loopback)...")
                try:
